from dictalchemy.tests import DynamicRelationChild
from dictalchemy.tests import DynamicRelationParent
from dictalchemy.tests import M2MAssociationDictParent
from dictalchemy.tests import M2MAssociationListChild
from dictalchemy.tests import M2MAssociationListParent
from dictalchemy.tests import M2mLeft
from dictalchemy.tests import M2mRight
from dictalchemy.tests import MultipleChildChild1
from dictalchemy.tests import MultipleChildChild1Child
from dictalchemy.tests import MultipleChildChild2
from dictalchemy.tests import MultipleChildParent
from dictalchemy.tests import Named
from dictalchemy.tests import NamedOtherColumnName
from dictalchemy.tests import NamedWithSynonym
from dictalchemy.tests import OneToManyChild
from dictalchemy.tests import OneToManyParent
from dictalchemy.tests import OptionalChild
from dictalchemy.tests import OrderingChild
from dictalchemy.tests import OrderingParent
from dictalchemy.tests import ParentWithOptionalChild
from dictalchemy.tests import TestCase
from dictalchemy.tests import WithAttributeMappedCollection
from dictalchemy.tests import WithAttributeMappedCollectionChild
from dictalchemy.tests import WithDefaultInclude
from dictalchemy.tests import WithHalChild
from dictalchemy.tests import WithHalParent
from dictalchemy.tests import WithHybrid
from dictalchemy.tests import WithMethodWithExtraArgumentChild
from dictalchemy.tests import WithMethodWithExtraArgumentParent
...
class TestAsdict(TestCase):
    ...
    def test_one_to_many_follow_with_parent(self):
        ...
        self.session.add(parent)
        self.session.commit()
        assert parent.asdict(follow={"child": {"parent": "relationships",},}) == {
            "id": parent.id,
            "name": parent.name,
            "relationships": {
                "child": child.asdict(),
    ...
    def test_multiple_child_follow_two_with_different_arguments(self):
        p, c1, c2 = self._setup_multiple_child()
        assert p.asdict(follow={"child1": {}, "child2": {"exclude": ["id"]}}) == {
            "id": p.id,
            "name": p.name,
            "child1": {"id": c1.id, "name": c1.name},
            "child2": {"name": c2.name},
    ...
    def test_multiple_child_follow_two_levels_with_arguments(self):
        p, c1, c2, c1c = self._setup_multiple_child_child()
        assert p.asdict(
            follow={"child1": {"follow": {"child": {"exclude": ["id"]}}}}
        ) == {
            "id": p.id,
            "name": p.name,
            "child1": {"id": c1.id, "name": c1.name, "child": {"name": c1c.name}},
        }
    ...
    def test_attribute_mapped_collection(self):
        ...
        assert p.asdict(follow=["childs"]) == {
            "childs": {"child1": {"parent_id": None, "id": None, "name": "child1"}},
            "id": None,
        }
    ...
    def test_dynamic_relation(self):
        ...
        parent = (
            self.session.query(DynamicRelationParent).filter_by(id=parent_id).first()
        )
        assert parent.asdict(follow={"childs": {}})["childs"][0]["id"] == child_id
    ...
    def test_method_argument(self):
        ...
        result = parent.ashal(follow={"child": {"_embedded": True, "method": "asdict"}})
        expected = {
            "id": parent.id,
            "child_id": child.id,
            "_embedded": {
    ...
    def test_extra_kwargs(self):
        ...
        assert parent.asdict(follow=["child"], method="extra_method", number=123) == {
            "id": parent.id,
            "child_id": child.id,
            "child": {
                "number": 123,
    ...
    def test_orderinglist(self):
        ...
        assert parent.asdict(
            only=["children"], follow={"children": {"only": ["position"]}}
        ) == {
    ...
    def test_follow_m2m_association_list(self):
        ...
        assert parent.asdict(only=["child_list"], follow={"child_list": {}}) == {
            "child_list": [2, 7]
        }
    ...
    def test_follow_m2m_association_dict(self):
        ...
        assert parent.asdict(only=["child_dict"], follow={"child_dict": {}}) == {
            "child_dict": {
                "key a": "value a",
                "key b": "value b",
            },
